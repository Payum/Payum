...
* Monetary values are expressed as `moneyphp/money` objects. Commands take a `Money` where they took minor units, results report one, `Payum\Core\Handler\Context::amount()` resolves "how much is this execution for" against the subject, and `Payum\Core\Model\Payment` and `Payout` answer `getMoney()`. Every integer accessor keeps working and keeps its meaning, so nothing has to change at once
* `Payum\Core\Model\MoneyAwareInterface` is how an application's own entity opts in, and `Payum\Core\Money\Amount` reads and writes an amount on a model whether or not it has. Both map over `totalAmount` and `currencyCode`, so **no storage or Doctrine mapping changes**: those two columns already hold everything a `Money` is made of
* Add a `Money\Currencies` service, with `Money\MoneyFormatter` and `Money\MoneyParser` defined against it. It knows ISO 4217 and moneyphp's crypto currencies out of the box, and an application registers its own through `PayumBuilder::addGlobalService()` — which is what makes a currency with 8 or 18 decimal places work rather than being rounded to 2 (#837, #1062)
* `Payum\Core\Action\GetCurrencyAction` answers for a currency ISO 4217 does not list by taking its exponent from that service, so an unported 1.x gateway handles one too. It takes an optional `Money\Currencies` again
* The PayPal Express Checkout, Pro Checkout and Masspay, Sofort and Authorize.Net AIM convert actions now send an exact decimal string rather than a float divided out by hand, which is what made a three-decimal currency lose a digit
* The 1.x information requests are answered from the services 2.0 injects, so a gateway still dispatching them keeps working with nothing registered by hand while it is ported. `GetHttpRequest` is filled from the PSR-7 `Psr\Http\Message\ServerRequestInterface` the container holds, `RenderTemplate` renders through `Payum\Core\Template\RendererInterface`, `GetToken` reads the token storage and `GetCurrency` reads ISO 4217. `ObtainCreditCard` stays with the framework integration. A ported action dispatches none of them: a handler is given the same information on `Payum\Core\Handler\Context` and names its template by returning a `Payum\Core\Result\NextAction\RenderTemplate` result
* `Payum\Core\Bridge\PlainPhp\Action\GetHttpRequestAction` and `Payum\Core\Bridge\Twig\Action\RenderTemplateAction` are deprecated and need removing from your config rather than replacing: core answers both requests on its own. Neither reads PHP's superglobals any more, so an application registering its own PSR-7 request, Twig environment or layout has it honoured by unported actions too
...
* Add `Payum\Core\Legacy\LegacyGatewayAdapter`, which drives a 1.x gateway — its factory and all its actions — with 2.0 commands, so an application that has moved to commands keeps access to the gateway packages that have not. Commands become the 1.x requests that mean the same thing, thrown replies become next actions, and the status is read back from the gateway's own status action. Redirect flows are unaffected: the token is passed through, so the same command still finishes the payment when the customer returns. A command carrying an amount, a reason or an idempotency key is refused rather than silently run without it, and `capabilities()` reports a best-effort list worked out from the actions the gateway registered
* Add `Payum\Core\Legacy\LegacyReply`, a next action carrying a 1.x reply 2.0 has no vocabulary for — the `HttpResponse` a 1.x action throws to render its own card form — and `Payum\Core\Gateway::supportsRequest()`, which answers whether any registered action claims a request without executing it
...
* Every reply 1.x throws now has a next action and back again. `Payum\Core\Legacy\ReplyToNextAction` translates a thrown reply into what a caller acting on a `Result` reads, and `ResultToReply` translates it back — the two are inverses, so a reply that crosses the boundary comes back the reply it was, including one a gateway declares for itself, which is carried across as a `LegacyReply` rather than guessed at
* A `Challenge` or a `Poll` reaching a 1.x caller now degrades to the journey 1.x did have — a form posted at the issuer for a step-up, and nothing thrown for a poll, which sends the caller to the after url to read the pending status — instead of throwing at it
