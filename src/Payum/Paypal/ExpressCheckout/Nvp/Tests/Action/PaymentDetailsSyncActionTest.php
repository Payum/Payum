...
class PaymentDetailsSyncActionTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testShouldRequestGetExpressCheckoutDetailsAndUpdateModelIfTokenSetInModel()
    {
        ...
        $gatewayMock
            ->expects($this->once())
            ->method('execute')
            ->with($this->isInstanceOf(\Payum\Paypal\ExpressCheckout\Nvp\Request\Api\GetExpressCheckoutDetails::class))
    ...
    public function testShouldRequestGetTransactionDetailsTwice()
    {
        ...
        $gatewayMock
            ->expects($this->at(1))
            ->method('execute')
            ->with($this->isInstanceOf(\Payum\Paypal\ExpressCheckout\Nvp\Request\Api\GetTransactionDetails::class))
